...
export default function ProfilePage() {
	...
	function changeLanguage(language) { update("language", language); setLanguage(language); if (settings.voice.language === settings.language) updateNested("voice", "language", language); }
	...
	return <main className="min-h-screen bg-[#f7fbf8] dark:bg-slate-950 px-4 py-10 sm:px-6"><form onSubmit={saveChanges} className="max-w-4xl mx-auto space-y-6">
		...
		<Section title={t.voice} description="MediLens reads existing report information only. It does not provide medical advice."><div className="divide-y divide-slate-100 dark:divide-slate-800"><Toggle label={t.readAloud} checked={settings.voice.enabled} onChange={(value) => updateNested("voice", "enabled", value)} /><Toggle label={t.autoRead} description="Nothing plays unless you explicitly enable this option." checked={settings.voice.autoReadSummary} onChange={(value) => updateNested("voice", "autoReadSummary", value)} /></div><div className="mt-5 grid gap-5 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t.voiceLanguage}<select value={settings.voice.language} onChange={(event) => updateNested("voice", "language", event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-2.5 text-sm">{Object.entries(LANGUAGE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-sm font-medium text-slate-700 dark:text-slate-300">{t.speed}<input type="range" min="0.5" max="2" step="0.1" value={settings.voice.rate} onChange={(event) => updateNested("voice", "rate", Number(event.target.value))} className="mt-4 w-full accent-emerald-600" /><span className="text-xs text-slate-500">{settings.voice.rate}x</span></label></div></Section>
